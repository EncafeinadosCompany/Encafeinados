"use client"
import { MapPin, Phone, Star, ArrowRight, Edit, Globe } from "lucide-react"
...
export function BranchCard({ branch, onViewDetails, onEdit, index = 0 }: BranchCardProps) {
    ...
    >
      <Card className="overflow-hidden border-none shadow-md hover:shadow-lg transition-all duration-300 bg-white h-full flex flex-col relative rounded-lg">
        ...
        <div className="absolute top-3 right-3 z-10">
          <div className={`flex items-center gap-1.5 px-2 py-1 rounded-full ${statusConfig?.bgColor} ${statusConfig?.textColor} text-xs font-medium shadow-sm`}>
        ...
        <div className="p-4 border-b border-[#F5E4D2]">
          ...
          {branch.average_rating && (
            <div className="flex items-center gap-1.5 mb-3">
              ...
              <span className="text-sm font-medium text-[#DB8935]">{branch.average_rating}</span>
            </div>
          )}
        ...
        <CardContent className="p-4 flex-grow space-y-4">
          ...
          <div className="flex items-center gap-2 text-[#A67C52]">
            ...
            <span className="text-sm">{branch.phone_number || "No disponible"}</span>
          </div>
          ...
          {branch.social_branches && branch.social_branches.length > 0 && (
            <div className="border-t border-[#F5E4D2] pt-3">
              <p className="text-xs font-medium text-[#DB8935] mb-2">Redes Sociales</p>
