...
export const BranchListWidget = ({
  ...
  const handleNavigateToRegister = useCallback(() => {
    navigate("/stores/register/branch");
  }, [navigate]);
  ...
  const viewProps = useMemo(() => ({
    ...
    showActions,
    isLoading,
  }), [paginatedBranches, onViewDetails, onAssingBranch, onQR, onVisit, showActions, isLoading]);
  ...
  const tableViewProps = useMemo(() => ({
    ...
    sortDirection,
    onSort: handleSort,
  }), [viewProps, sortField, sortDirection, handleSort]);
    ...
    <TooltipProvider delayDuration={300}>
      <div className="space-y-4 p-1">
        ...
        <div className="min-h-[300px] bg-white/30 backdrop-blur-sm rounded-xl border border-[#E6D7C3]/20 overflow-hidden">
          {viewMode === "card" ? (
            <BranchCardsView {...viewProps} />
          ) : (
            <BranchTableView {...tableViewProps} />
          )}
        </div>
