...
import {
  EditBranchesSchemas,
  EditBrancheType,
} from "@/common/utils/schemas/auth/edit_branches.schema";
...
export default function FormEditBranchWidget() {
  ...
  const onSubmit = async (data: EditBrancheType) => {
    try {
        if(selectedBranchId){
            useUpdateBranches({ id: selectedBranchId, data });
        }
    } catch (error) {
  ...
  const onLocationSelect = useMemo(
